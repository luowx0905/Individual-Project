...
In a similar study to this project, \citet{RN17} devised a reliable method for predicting the prices in Riyadh, Saudi Arabia. The attributes of properties, such as location, area, type and price, are identical to the dataset in this project, whereas the interior layouts and facilities, such as parking spaces, outside areas, and heating were not covered in their study, hence the dataset in their study was inferior to the one in this project. Furthermore, their study based on the properties in middle east country, which is significantly different from the UK in all aspects, such as, culture and climate, suggesting that the results from their study was likely to be unapplicalbe in the current project. Various models were utilized, including decision tree, random forest, and linear regression. For evaluation stage, the mean absolute error, mean squared error and median squared error were applied. As a result, the random forest has the best performance among three algorithms. 
\\
