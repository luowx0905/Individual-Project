...
In the dataset, four features are characterized by categorical keywords, including parking, heating, accessibility, and outdoor spaces. Figure \ref{parking_dataset} is a screenshot of \textit{parking}, which is used as an example to demonstrate the manipulation of the keywords. It indicates that there is one parking space for the first property, which is allocated off-street parking for residents, the second row has none, and the third property has one on-street parking space.
\begin{figure}[h]
	\centering
...
A class, \textit{\textbf{GeneralizeDataset}}, was developed to obtain the number of  keywords in each row, and its UML diagram is illstrated in figure \ref{uml_generalize_dataset}. The core of this class is member function \textit{\textbf{get\_feature\_types}}, and its flowchart is displayed in figure \ref{generalize_dataset_get_feature_types}.
\begin{figure}[h]
	\centering
