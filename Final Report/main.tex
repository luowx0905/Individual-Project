...
In another research, \citet{RN20} developed a machine learning model for forecasting property prices. The dataset they used is updated by the Florida authority, and they collected the transactions between 2010 and 2019 which includes around 100000 records. Additionally, the economic trends, such as gross domestic product (GDP), custom price index (CPI) were obtained and integrate with the property transactions \cite{RN20}. Their dataset is superior to the one in the current project since there are more records, and the local economics were also considered in their research. To accurately solve the problem, they employed a variety of machine learning algorithms, including logistic regression, random forest, volting classifier, and XGBoost. 
