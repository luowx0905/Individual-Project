...
The datasets produced by CreateInputDataset were initially loaded, and data standardization was applied so that all the data were on the same scale. The dataset was then shuffled to eliminate the possibility that  it contains intrinsic patterns. Next, both features and labels are divided into two parts, one for training and the other for validation.
\\

The architecture of the first model developed in this project only consists of linear layers and activation functions, as depicted in figure \ref{basic_model_layout}. There are two outputs of the model, which are prices and probabilities of selling. Therefore, the activation functions of the output layers are ReLU and sigmoid, respectively.
\\
