...
\subsection{Basic Model: Price Excluded}
Based on the training process in \ref{basic_status_training_included}, this model is directly trained with baises of 0, 0.05, 0.1 and 0.15, the corresponding losses are depicted in figures \ref{basic_status_exclude_0} to \ref{basic_status_exclude_015}. Folds zero, one, zero, and three of each bias are selected for evaluation due to the low loss. 
...
Figure \ref{cm_exclude} and table \ref{metrics_exclude} illustrate the confusion matrices and metrics of each model trained with various biases. 
