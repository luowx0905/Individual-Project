...
\textit{\textbf{TensorFlow}} is also an ML framework produced by Google that specializes in deep learning and neural networks. One of the attractive features of this library is that it supports automatic differentiation, which can be advantageous for the model training. Another appealing characteristic of \textit{TensorFlow} is called \textit{\textbf{TensorBoard}}, which is an interactive visualization system that can display the flowchart of the data manipulation and plot the tendency of the performance \citep{RN15}. 

\subsection{PyTorch}
\textbf{\textit{PyTorch}} is one of the popular ML frameworks developed by Facebook, which is designed to implement neural networks with flexibility and speed. It provides various components for model construction and training. For instance, there are numerous types of modules that comprise a model, such as linear layers, dropout, and activation functions, as well as a variety of loss functions and optimizers that can be employed in model training \citep{RN5}. 
