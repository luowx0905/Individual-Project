...
K nearest neighbor (KNN) is one of the classification algorithms for supervised learning, which stores all the training data \citep{RN33}. During forecasting, the distances between input and all the training data are calculated, and the class of the nearest training data would be the prediction. The distinct advantage of this algorithm is that it can be implemented effortlessly. In contrast, the disadvantage is obvious; the prediction could be time-consuming as the distances to all the training data are computed. 
